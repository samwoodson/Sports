...
class EspnScraper 
  ...
  def self.get_score
    ...
    #basketball = ESPN.get_nba_scores( Date.parse('Apr 24, 2016') )
    hockey = ESPN.get_nhl_scores( Date.parse('Apr 24, 2016') )
    ...
    hockey.each do |ball|
      ...
      puts away_score
      puts date
      Score.create(home_team: home_team, home_score: home_score, away_team: away_team, away_score: away_score, date:date, sport:'hockey')
    end
